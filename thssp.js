...
client.on('message', async function(msg) { /* 봇에 메시지가 올 때 */
	...
	if( 
		...
		if(warningList[msg.member.toString()] == 1) {
			...
			// 1.4 문장(다른 사람이 작성함) - msg.channel.send('욕설을 사용했습니다. 15분 후에 자동적으로 차단해제 됩니다 한번 더 하실 경우엔 <정보 치환>관리자가 추방 처리하겠습니다 만약 테스트용으로 하셨을경우엔  <정보 치환>#차소게(<#<정보 삭제됨>>)에서 <정보 치환>관리자에게 요청하십시오.');
			// 1.0 문장(본인이 작성함) - msg.channel.send('욕설을 사용했습니다. 15분 후에 <정보 치환>관리자에게 차단 해제를 요청하십시오.');
